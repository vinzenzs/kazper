...
- [ ] 1.1 Create `internal/workoutcompliance/types.go`: response types —
...
- [ ] 1.2 Add pure expansion in `service.go`: flatten the effective program's step
  tree (repeat `count` → consecutive copies, one level deep) carrying flat
  `step_index` + repeat provenance; unit-testable without a DB.
- [ ] 1.3 Add pure scoring functions: metric selection by resolved target kind
...
- [ ] 1.4 Add `Service` with narrow injected interfaces (`workoutsRepo.GetByID`,
  ...
  mismatch.
- [ ] 1.5 Add `handlers.go`: `GET /workouts/:id/compliance` with swag annotations
...
- [ ] 2.1 Instantiate and register in `internal/httpserver/server.go` `Run()`:
...
- [ ] 3.1 Add a `workout_compliance` registry entry in `internal/agenttools/`
...
- [ ] 3.2 Regenerate the announced-schema golden for the new tool:
...
- [ ] 4.1 Unit-test expansion: single steps, `repeat ×N` ordering + provenance
  (interval 3 of 5 nameable), mixed programs; property: expanded length =
  Σ(singles) + Σ(count × group size).
- [ ] 4.2 Unit-test scoring tables: under/over/in-band per kind incl. the
  ...
  scorable.
- [ ] 4.3 Handler integration tests (testcontainers, per-handler convention):
...
- [ ] 4.4 Verify the MCP e2e/golden suite covers the new tool announcement
  (`go test -count=1 -tags=integration ./internal/mcpserver/...`).

## 5. Docs & verification

- [ ] 5.1 Run `task swag` to regenerate `docs/` for the new endpoint + response
  types.
- [ ] 5.2 Run `task vet` and `go test -count=1 ./internal/workoutcompliance/...
