# workout-stats Specification

## Purpose

Define a read-side aggregation of completed workouts into per-day and windowed volume totals with a by-sport breakdown, exposed at `GET /workouts/summary` and mirrored as the `training_totals` MCP tool. Workout distance, elevation, and duration are unit-isolated — they never feed nutrition, hydration, or energy totals.
## Requirements
