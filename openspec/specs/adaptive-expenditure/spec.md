# adaptive-expenditure Specification

## Purpose

Estimate the athlete's *actual* average daily energy expenditure (TDEE) by energy balance over
the two series Kazper already logs — meal intake and the body-weight capability's smoothed
trend — instead of resting nutrition targets on an assumed number nobody measured. Over a
window, `intake − expenditure = Δ stored energy ≈ Δ trend-mass × 7700 kcal/kg`; rearranged, the
expenditure falls out of data the athlete produced by logging normally.

The capability is defined as much by what it refuses to do as by the arithmetic. It never reads
a day it wasn't given: a day with no meals is excluded and counted, never read as zero intake.
It never guesses across thin data: below 14 logged days or 5 weigh-ins it returns a null
estimate and a reason rather than a confident wrong number. It never smooths body weight itself
— that stays the body-weight capability's one implementation. And it is advisory: it reads no
goals and writes nothing, so comparing the estimate against a configured target, and deciding
whether to move that target, remain the consumer's call through the deliberate goals PUT.
