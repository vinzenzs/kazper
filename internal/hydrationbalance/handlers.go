...
// Handlers wires the hydration-balance endpoints.
...
func (h *Handlers) Register(rg *gin.RouterGroup) {
	rg.POST("/hydration-balance", h.upsert)
	rg.GET("/hydration-balance", h.list)
	rg.GET("/hydration-balance/:date", h.get)
	rg.DELETE("/hydration-balance/:date", h.delete)
}
...
// @Tags         hydration-balance
// @Accept       json
// @Produce      json
// @Param        Idempotency-Key  header  string    false  "Optional client-supplied idempotency key"
// @Param        body             body    Snapshot  true   "Hydration-balance snapshot (date required; metrics optional)"
// @Success      201  {object}  Snapshot  "INSERT"
// @Success      200  {object}  Snapshot  "UPDATE (date already present)"
// @Failure      400  {object}  map[string]string  "date_invalid | sweat_loss_ml_invalid | activity_intake_ml_invalid | goal_ml_invalid"
// @Security     BearerAuth
// @Router       /hydration-balance [post]
...
func (h *Handlers) list(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"hydration_balance": out})
}
...
// @Failure      404  {object}  map[string]string  "hydration_balance_not_found"
// @Security     BearerAuth
// @Router       /hydration-balance/{date} [get]
func (h *Handlers) get(c *gin.Context) {
	out, err := h.svc.Get(c.Request.Context(), c.Param("date"))
	if err != nil {
		if errors.Is(err, ErrNotFound) {
			respondError(c, http.StatusNotFound, "hydration_balance_not_found")
			return
		}
...
// @Failure      404  {object}  map[string]string  "hydration_balance_not_found"
// @Security     BearerAuth
// @Router       /hydration-balance/{date} [delete]
...
func respondServiceError(c *gin.Context, err error) {
	for _, e := range []error{
		ErrDateInvalid, ErrSweatLossMLInvalid, ErrActivityIntakeMLInvalid, ErrGoalMLInvalid,
	} {
		if errors.Is(err, e) {
...
// round applies 1dp rounding to the ml metrics at the response boundary.
func round(s *Snapshot) *Snapshot {
	if s == nil {
		return nil
	}
	out := *s
	out.SweatLossML = numfmt.Round1Ptr(s.SweatLossML)
	out.ActivityIntakeML = numfmt.Round1Ptr(s.ActivityIntakeML)
	out.GoalML = numfmt.Round1Ptr(s.GoalML)
	return &out
}
