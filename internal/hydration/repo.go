...
// ListByWorkout returns every hydration entry tagged with the given
...
func (r *Repo) ListByWorkout(ctx context.Context, workoutID uuid.UUID) ([]*Entry, error) {
	rows, err := r.q.Query(ctx,
		`SELECT `+selectCols+` FROM hydration_entries WHERE workout_id = $1 ORDER BY logged_at ASC`,
		workoutID,
	)
	if err != nil {
		return nil, fmt.Errorf("list hydration entries by workout: %w", err)
	}
	defer rows.Close()
	var out []*Entry
	for rows.Next() {
