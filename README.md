...
**Per-slot target + duration overrides + the effective program.** A slot may carry an optional
...
`hr_zone`/`power_zone` 1–5, `hr_bpm`, `power_w`, `rpe`). A slot may likewise carry a
`duration_overrides` list — `{intent, duration}` entries, at most one per intent — that supersede
the template's step **durations** (the same way, by `intent`), so one template can progress in
*length* too (e.g. a tempo block 75min one week, 80min the next). The `duration` reuses the
workout-template duration shape restricted to the bounded kinds (`{"kind":"time","seconds":…}` or
`{"kind":"distance","meters":…}`; `open`/`lap_button` are rejected). When a slot's effective
program is fully time-bounded, **materialize derives the planned session length from it**, so the
calendar block and the watch workout agree. `GET /workouts/{id}/program` returns a planned
workout's **effective program**: its template steps with the slot's target and duration overrides
applied (a workout with no template returns its sport/name and an empty step list). The Garmin watch
push compiles from this effective program. Each override list replaces wholesale on `PATCH`
(`[]` clears, omitting leaves unchanged).
...
| `plan_race_fueling`           | `GET /races/{id}/fueling-plan?body_weight_kg=…&sweat_rate_ml_per_hr=…&weather=…` | Deterministic per-leg in-event fueling baseline: carbs/sodium/fluid per hour + total per leg. Carbs band by total duration × discipline; fluid/sodium from sweat rate (flagged default otherwise). Optional `weather=true` scales fluid/sodium by the race-day forecast heat (bounded ~1.0–1.5×, carbs untouched, defaults stay flagged); byte-identical without it. |
| `plan_race_pacing`            | `GET /races/{id}/pacing-plan?weather=…`          | Deterministic per-leg pacing baseline from athlete-config thresholds: bike power band (%FTP), run pace band (vs threshold pace), swim pace band per 100 m (vs CSS), each duration-banded, with per-leg IF/estimated TSS + a race TSS total. Unset thresholds degrade only the affected legs. A duration-generic baseline, not course-specific. |
...
| `add_plan_slot`               | `POST /training-plans/{id}/weeks/{weekId}/slots` | Add a day-slot (weekday + template + optional time_of_day + optional per-intent `target_overrides` and `duration_overrides`). |
| `patch_plan_slot`             | `PATCH /training-plans/{id}/slots/{slotId}` | Update a slot's weekday / ordinal / template_id / time_of_day / target_overrides / duration_overrides (each list replaces wholesale; empty clears). |
...
| `power_profile`               | `GET /workouts/power-profile?from=…&to=…&weight_kg=…&sex=…` | Rank the window's best power efforts against the Coggan tables at four anchors (5 s / 1 min / 5 min / 20 min-as-FT): W/kg + category band + interpolated percentile, plus a rider phenotype (sprinter/time-trialist/climber/all-rounder, null unless all four present). Weight = `weight_kg` or latest stored (else `weight_data_missing`); `sex` picks the table (male default). Advisory. |
...
| `workout_fueling_plan`        | `GET /workouts/{id}/fueling-plan?carbs_per_hr=…` | Pre-session plan for a **planned** workout: work `kJ = planned_tss/100 × effective FTP × 3.6`, carb burn via the IF→CHO ladder (45/55/70/80%), intake from the duration ladder (0 / 30–60 / 60–90 g/hr) clamped by tested gut capacity, plus `projected_deficit_g`. Degrades to intake guidance on `tss_missing`/`ftp_missing`; completed → 409. |
...
| `fuel_plan`                   | `GET /nutrition/fuel-plan?from=…&to=…&tz=…` | Periodized carb targets from planned load (default today + 6 days). Day tier from planned TSS — rest/easy/moderate/heavy at 0 / <60 / 60–150 / >150 (or any session ≥ 150 min) → a 3/5/7/9 g/kg ladder × the weight trend, beside the date's effective goal + delta. `plan_missing` marks days the plan doesn't reach. Suggestions only — applying is the override PUT. |
| `log_location_period`         | `POST /locations`                      | Record where the athlete is over a date range ("Mallorca, July 20–28") so weather/heat reads resolve there instead of home. Pass `place` (geocoded server-side) or explicit `lat`/`lon`. Overlaps fine — latest start wins. No edit: delete + re-log. |
| `list_location_periods`       | `GET /locations?from=…&to=…`           | Travel periods overlapping a window, ascending. Days with no period are spent at the configured home (`HOME_LAT`/`HOME_LON`). |
| `workout_heat`                | `GET /workouts/{id}/heat`              | Heat picture for a **planned** session: composite `heat_load_c` (heat index + solar penalty scaled by cloud − bounded wind cooling), acclimatization derived from the athlete's own recent outdoor sessions (with the qualifying ids), a suggested % off effective FTP/threshold pace, and a fluid note. Indoor → `not_applicable`; degrades on `location_unconfigured`/`weather_unavailable`. A heuristic, not WBGT — advisory only. |
| `heat_analytics`              | `GET /workouts/heat-analytics?from=…&to=…&tz=…` | How heat has actually affected this athlete: outdoor completed workouts bucketed by session heat index (`<20`/`20-25`/`25-30`/`>30` °C) with mean duration/EF/decoupling/output-vs-baseline, plus Spearman rho for EF and decoupling (gated at 10 pairs). Indoor excluded; `assumed_outdoor` tallied. The evidence stream for refining the heat constants — descriptive, never an auto-refit. |
...
| `daily_context`               | `GET /context/daily?date=…&tz=…`       | One call returns adherence + totals + hydration + today's workouts + fuel + weight + phase + goal-override presence + today's wellness entry. Recommended first call of a session. |
| `log_wellness`                | `PUT /wellness/{date}`                 | Record the athlete's subjective daily wellness — five optional 1–5 scores (`fatigue`/`soreness`/`stress`: 1 none→5 severe; `mood`/`motivation`: 1 low→5 high) plus a `note`. Full-replace; at least one field required. The self-reported half of recovery, beside the Garmin vitals. |
| `list_wellness`               | `GET /wellness?from=…&to=…`            | List wellness entries over a window (92-day cap), ascending — hold subjective trend against the objective load/recovery data. |
| `log_supplement`              | `POST /supplements`                    | Record a supplement intake (creatine, iron, vitamin D, out-of-session caffeine). `name` required; `dose`+`dose_unit` paired or both omitted; optional `note`. Its own log — not meals (no macros), not workout-fuel (in-session) — and feeds no nutrition total. No edit: correct by deleting and re-logging. |
| `list_supplements`            | `GET /supplements?from=…&to=…`         | List supplement intakes over a window (92-day cap), ascending — check protocol adherence across a block. |
