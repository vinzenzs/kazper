...
_Last refreshed: 2026-06-16 by the `continuity` skill (the 5-change workout-target arc is COMPLETE — all archived; `openspec/changes/` is empty, nothing in flight or queued)._
...
_Nothing in flight; tree clean on `main`. `openspec/changes/` is empty._

## Up next

Ordered queue — top is next to pick up.

_(empty — no proposals open. The next change has to be proposed first via `/opsx:propose`. Strongest candidate: **multisport Phase 2** — see Backlog.)_

## Backlog

Planned changes not yet prioritized.

- _No open proposals._ Future-but-unproposed candidates:
  - **multisport Phase 2** — plan-slot/materialize integration + `multisport`/`transition` in `workouts.Sport`/`Program`, so a triathlon/brick can be planned and materialized like single-sport workouts (Phase 1 shipped only the standalone library + compile + schedule). Propose once Phase 1 settles in real use.
  - **IF-from-FTP consumption** — derive `intensity_factor` from `normalized_power_w / ftp_watts` (still deferred per the athlete-config spec; the cadence + secondary-target consumptions are now done).
...
- **The workout-target arc is COMPLETE — 5/5 shipped + archived** (surfaced 2026-06-15 from the Garmin step-editor screenshots):
  ...
  - `add-multisport-structured-workouts` (archived 2026-06-16) — **Phase 1**: new `multisport-workouts` capability (per-sport segments + transitions, `multisport_templates` table, migration `045`), garmin-bridge multi-segment compile, compile-and-schedule action. **Phase 2 (plan/materialize integration) deferred — see Backlog.**
...
- **The "mirror everything" Garmin arc: COMPLETE — archived** (`add-garmin-{workout-detail,daily-energy,gear-and-prs,athlete-config,misc-mirror,history-backfill,sync-rolling-lookback}` + `garmin-workout-library-mgmt`, plus `extend-recovery-fitness`). Migrations 036–041 landed; **multisport Phase 1 added `045_add_multisport_templates`, so head is now `045` on disk.** Re-verify the head before any future `task migrate:new`.
...
- **Drift to clean up (carried):**
  - **`main` is well ahead of `origin/main`** — the whole prior arc plus the full 5-change workout-target arc is local-only. Push when ready.
