...
_Last refreshed: 2026-06-16 by the `continuity` skill (`persist-coach-recommendations` shipped & archived — priorities #6F is now done; `openspec/changes/` is empty — nothing in flight or queued; `main` is ahead of `origin/main`)._
...
- **Drift to clean up (carried):**
  - **`main` is ahead of `origin/main`** — the `persist-coach-recommendations` feat+archive commits plus this continuity/roadmap doc sync are local-only (origin advanced since the last push, so the exact count is small). Push when ready.
