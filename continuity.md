...
_Last refreshed: 2026-06-21 by the `continuity` skill (`add-workout-training-focus` shipped & archived — migration head is now `048`; one open proposal, `add-macrocycle-planning`, is mid-draft in Up next; `main` is ahead of `origin/main`)._
...
1. **add-macrocycle-planning** — `Why:` the system models a single training block (a typed `training-phase` date range) but has no concept of the *season* those blocks add up to — no yearly container turning a pile of independent phases into one goal-anchored progression toward an A-race. _Why now: only open proposal. **Mid-draft — has proposal/design/specs but no `tasks.md`, and is uncommitted/untracked**; finish artifacts (re-run `/opsx:propose`) before `/opsx:apply`._
...
- **Drift to clean up (carried):**
  - **`main` is ahead of `origin/main`** — the `add-workout-training-focus` feat+archive commits plus this continuity/roadmap doc sync (and any prior unpushed work) are local-only. Push when ready.
  - **`add-macrocycle-planning` is uncommitted/untracked** — its proposal/design/specs sit in the working tree (no `tasks.md` yet). Commit or finish it when you pick it up.
